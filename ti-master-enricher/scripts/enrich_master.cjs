...
async function main() {
    ...
    // Using relative paths to the root scripts folder for better reliability in some environments
    const scripts = {
    ...
    if (results.vt && !results.vt.error) {
        maxScore += 20;
        // Logic depends on the VT script output format, usually look for positives or malicious
